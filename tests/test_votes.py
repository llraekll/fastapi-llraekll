...
def test_delete_vote_non_exist(authorized_client, test_posts, test_vote):
    res = authorized_client.post("/vote/", json={"post_id": test_posts[3].id, "dir": 0})
    assert res.status_code == 201

def test_vote_post_non_exist(authorized_client, test_posts, test_vote):
    res = authorized_client.post("/vote/", json={"post_id": 87565, "dir": 1})
    assert res.status_code == 404

def test_vote_post_unauthorized_user(client, test_posts, test_vote):
